...
const createFile = async () => {
  try {
    ...
    await useTauriFsWriteTextFile(
      inputState.value.fileName!,
      inputState.value.fileContent!,
      {
        baseDir: useTauriFsBaseDirectory.Document,
      },
    );
  ...
  >
    ...
    >
      <UFormField label="Text file name (with extension)" name="fileName">
        <UInput
          v-model.lazy.trim="inputState.fileName"
          variant="subtle"
          size="lg"
        />
      </UFormField>
      ...
      <UFormField label="File content" name="fileContent">
        <UTextarea
          v-model.lazy="inputState.fileContent"
          variant="subtle"
          size="lg"
          :rows="10"
        />
      ...
      <UButton type="submit" size="lg"> Create file </UButton>
    </UForm>
  </LayoutTile>
</template>
